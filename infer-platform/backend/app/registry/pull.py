...
def _pull_oci(image: str, digest: str | None = None) -> dict[str, Any]:
    ...
    auth = (settings.MODEL_REGISTRY_USER, settings.MODEL_REGISTRY_TOKEN) if settings.MODEL_REGISTRY_USER else None
...
def _get_manifest(
    client: httpx.Client, api_registry: str, repo: str, tag: str, auth: Any
    ...
    # digest 口径（跨平台契约 §2.4）：以仓库回执 Docker-Content-Digest 为准；A 侧镜像字节可复现，
    # 同 model_ref 同内容重推必得同一 digest → 本侧「同 ref 同 digest=ready / 不同 digest=40900」的
    # 幂等判定对 A 的任意时刻重推都稳定。回执缺失时用 manifest 内 config digest 兜底展示。
...
def list_remote_tags(repo: str | None = None, refresh: bool = False) -> dict[str, Any]:
    ...
    try:
        with httpx.Client(
            follow_redirects=True, timeout=httpx.Timeout(connect=5.0, read=30.0, write=30.0, pool=30.0)
        ) as client:
