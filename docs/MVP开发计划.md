...
| 模型分发 | A 把「ONNX + `model.yaml`」打成镜像 `docker push` 到 registry；B 用 Registry v2 API 拉取并校验；**无 A→B 直连、无实例管理、无心跳** |
| 工位与模板 | **全部由 B 自管**（工位/相机/工位模板，GUI 编辑）；A 不持有工位主数据，只在 `model.yaml` 给推荐阈值 |
| 工时 | 3×22 = 66 人日；五条主线保留；B 前端独立开发；A 侧报告最后做、可砍 |
| 里程碑 | M1(D8) 发布镜像→B 拉取→配模板→假图推理→错图回传→A 建复审项；M2(D14) 标注→训练→审批→发布→拉取→模板→推理→回传→复审→回流；M3(D19) 预标 + 真机 1 路 + 8 路仿真 + 日报 + 双机离线包；M4(D22) 验收 |
...
4. D9~12：训练链（数据集版本/划分/红线、LS data_export 包裹、训练执行流 + 门禁 + 金标准 + ONNX 导出、模型注册与审批）+ **模型发布服务**（生成 `model.yaml` → 构建镜像 → `docker push`）。
...
| `pipeline-core`（切片/NMS/合并/三档判定/`load_config`） | **C 主笔**，B 消费（预标） |
...
- **A 侧前端页面**：数据集、训练（含模型库与发布）、复审、系统（用户/角色/审计）（复用 LS 组件库）+ 素材更换（公司 logo/名称/描述，去除 LS 吉祥物）。
...
| 9 | `POST /api/prelabel/{task_id}/{health,setup,predict,validate}`（LS 官方协议） | LS→A | D15 ★ |
...
| D7 | 双机联调 + 离线包初版；B 前端「工位与相机」页 | **模型发布服务真实推送**（`model.yaml` → 镜像 → `docker push` → `model_publish` 状态） | **B 模型拉取真实打通**（Registry v2 manifest/层解包/校验/注册）；B 前端联调 |
...
| D10 | 万级导入压测；B 前端图表联调 | LS data_export 包裹（YOLO 导出 + 红线校验） | B 多模型按对象分发 + 模型库页（拉取/导入/删除） |
| D11 | 交付包完善（权重/字体/双平台镜像/离线模型镜像） | YOLO adapter + 训练执行流 + 训练进度 SSE + 发布前置（金标准/审批） | B 加载自检（张量匹配 + 空跑）+ 预加载/卸载 |
| D12 | 自检脚本；B 前端「模型库」区 | 门禁 + 金标准回归 + ONNX 导出 + 模型 lifecycle + 审批流 | B 磁盘保留策略 + 清理任务 + 磁盘水位告警 |
| D13 | 备份脚本 | 复审/建议清单/回流后端完整 | B 检测记录/错图统计接口完善 |
| D14 | **M2 联调** | **M2 联调** | **M2 联调** |
| **M2** | 标注 → 训练 → 审批 → **发布镜像** → B 拉取 → 工位模板 → 推理 → 回传 → A 复审 → 回流 全闭环（小数据集） | | |
